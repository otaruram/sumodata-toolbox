# Changelog

All notable changes to the SumoData Toolbox extension will be documented in this file.

## [0.1.0] - 2026-03-27

### Added
- Initial release of SumoData Toolbox
- 9 AI-powered tools for data professionals
- **Sumo Pipes (Data Engineering)**:
  - SQL Optimizer for database query optimization
  - JSON to DDL Schema generator
  - Cron Expression Generator
- **Sumo Lens (Data Analysis)**:
  - Complex Regex Explainer
  - Pandas Cleaning Logic Suggester
  - SQL Logic Explainer
- **Sumo Core (DS/ML)**:
  - Google-style Docstring Generator
  - Python Type Hinting
  - PyTorch/Scikit-learn Training Loop Boilerplate
- 6 AI model options (Claude, Gemini, GPT, Kimi)
- Sidebar UI with accordion organization
- Context menu integration for .py and .sql files
- Secure API key storage
- Comprehensive documentation

### Features
- One-click AI code generation and optimization
- Token-efficient prompts
- Multiple access methods (sidebar, context menu, command palette)
- Configurable timeout and code length limits
- Support for streaming responses
- Dark theme UI with cyan accents

### Security
- Secure API key storage using VS Code SecretStorage
- No code logging or storage
- Direct API communication with sumopod.com

---

## Roadmap

### v0.2.0 (Planned)
- Template library (save and share AI generations)
- Workflow automation (chain multiple tools)
- Usage analytics and metrics
- Improved error handling

### v0.3.0 (Planned)
- Team collaboration features
- Custom prompt templates
- Integration with Jupyter notebooks
- DBT and Airflow integrations

### v1.0.0 (Planned)
- Stable API
- Premium features
- Enterprise support
- Community marketplace

---

For detailed documentation, visit: [Documentation](https://otaruram.github.io/sumodata-toolbox-docs/)
